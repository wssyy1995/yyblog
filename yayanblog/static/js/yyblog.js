...
var change_picture=function(ind){
    for(let i=0;i<recommend_slide.length;i++){
            //for循环将图片list每个图片的z-index先恢复统一 z-index
            recommend_slide[i].style['z-index']=100;
        }

        // 接收希望跳转的索引参数，将对应的图片z-index设为100以上
        recommend_slide[ind].style['z-index']=101;

}

//定义一个更换小点颜色的方法，接收参数为希望跳转的图片和小点的索引
var change_dot=function(ind){
        for(let i=0;i<slide_trigger.length;i++){
                slide_trigger[i].style['background-color']='white';
            }
        slide_trigger[ind].style['background-color']='green';
...
function autochange(){
        ...
        // }
        //
        // recommend_slide[c_index].style['z-index']=110;
        //
        // for(let i=0;i<slide_trigger.length;i++){
        //             slide_trigger[i].style['background-color']='white';
        //         }
        // slide_trigger[c_index].style['background-color']='green';



}

// 创建一个定时器开启计时，传入轮播图函数，每2.5秒就会执行这个函数
var auto_picture=setInterval(autochange, 2500);
...
gallery_recommend_big.onmouseout=function(){
    ...
    auto_picture=setInterval(autochange, 2500);
